...
import {Pokemon, PokemonType} from "../../types/app";
import MenuItem from "@mui/material/MenuItem";
import axios from "axios";
import {enqueueSnackbar} from "notistack";

function PokemonForm(props: { openState: boolean, setStateFunc: Function, tableReloadFunc: Function, pokemon: Pokemon }) {
    ...
    function handleSubmit(event: React.FormEvent) {
        ...
        const method = pokemonID == 0 ? 'post' : 'put';
        const urlEdit = `http://localhost:3000/pokemons/${pokemonID}`;
        const urlCreate = `http://localhost:3000/pokemons`;

        axios({
            method: method,
            url: pokemonID == 0 ? urlCreate : urlEdit,
            data: newPokemon
        }).then(response => {
            ...
            if (status === 201 || status === 200) {
                ...
                props.setStateFunc(false);
                enqueueSnackbar(`Pokemon ${ pokemonID == 0 ? 'created' : 'edited' } successfully!`,
                    { variant: 'success', autoHideDuration: 1500, anchorOrigin: { vertical: 'top', horizontal: 'right' } }
                );
            } else {
                enqueueSnackbar(`Error ${ pokemonID == 0 ? 'creating' : 'editing' } pokemon!`,
    ...
    return (
        <Dialog open={props.openState} onClose={() => handleClosing()}>
            <Box sx={{ padding: 1 }}>
                {
                    pokemon.id === 0
                        ? <h1>Creating new pokemon</h1>
                        : <h1>Editing pokemon #{ pokemon.id }</h1>
                }

                <form onSubmit={handleSubmit} noValidate autoComplete="off" style={{ marginTop: 20 }}>
                    <Stack spacing={2} direction="row" sx={{ marginBottom: 4 }}>
                        ...
                        <FormControl variant="outlined" sx={{ m: 1, minWidth: 120 }}>
                            <InputLabel id="demo-simple-select-standard-label">Pokemon Type</InputLabel>
                            <Select
                                ...
                                required
                            >
                                <MenuItem value="" disabled={true} hidden={true} selected={true}>
                                    <em>None</em>
                                </MenuItem>
